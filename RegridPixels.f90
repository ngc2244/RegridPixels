PROGRAM RegridPixels
        ...
        CHARACTER*200 Input_Dir, Output_Dir
        ...
        INTEGER :: temp_YEAR, temp_MONTH, temp_DAY, temp_ID
        REAL    :: temp_SZA, temp_CF, temp_AMF
        ...
        !+++++++++++++++++++++++++++++++++++++++++++++++++++++++
        INTEGER :: i, row, col, p, year
        INTEGER :: Year_start, Year_end, Month_start, Month_end
        CHARACTER*4  cYear, cYear1, cYear2
        ...
        ! Open pixels file
        Input_Filename = TRIM(Input_Dir)//TRIM(Input_Filename)
...
13      CONTINUE
        ...
        READ(In_Unit,"(I8,13F15.6,2E15.6)",end=15,err=14) temp_ID,                          &
                LAT(p,1:5), LON(p,1:5), temp_SZA, temp_CF, temp_AMF,                        &
                VCD(p), VCD_Unc(p)
        GOTO 13
14      CONTINUE
...
15      CONTINUE
        ...
        Lat_low   = FLOOR(MINVAL(LAT))-1
        Lat_up    = CEILING(MAXVAL(LAT))+1
        Lon_left  = FLOOR(MINVAL(LON))-1
        Lon_right = CEILING(MAXVAL(LON))+1
